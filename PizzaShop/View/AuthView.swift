...
//  PizzaShop
//
//  Created by Никита Юрковский on 9.01.23.
//
...
struct AuthView: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 20) {
            Text(isAuth ? "Авторизация" : "Регистрация") // ? -  Тернарный оператор
...
//////////////////////////////////////////////////// Здесь было singIn в 10 уроке 5:30 минута (изменил/ было signUp)
                        AuthService.shared.signIn(email: self.email,
                                                  password: self.password) { result in
                            switch result {
                                
                            case .success(_):  //let user): так было пока не подключили юзеров
                ...
                Button {
                    isAuth.toggle() // Переключение состояние
                } label: {
